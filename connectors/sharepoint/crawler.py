    ...
    *,
    ...
    try:
        _file_id, was_new = await _run_blocking(
            ...
            stable_id=stable_id,
            # RESOLVED values from `_prepare_document` — the real path/name
            # for a plain scope, the anonymized ones for a marked scope. NOT
            # the raw `path`/`name` locals above: those are for routing and
            # logging only, and must never reach storage for a marked scope.
            path=prepared.path,
