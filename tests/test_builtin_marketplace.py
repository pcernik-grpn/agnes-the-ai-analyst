...
# It asserts `>Enabled<` and `plugin-enabled-toggle` are in the template;
# this branch replaces that toggle with the three-position reach control
# (Off / By group / Everyone), so the control it pins no longer exists and
# the test cannot pass here. The behaviour it protected — that the caption
# is not the inverted 'Disabled' — is asserted by the reach test below.
#
# Recorded rather than silently dropped because the merge would have
# dropped it either way: git aligned the two function signatures and let
# this branch's body serve as the continuation for both, so main's
# assertions were absent from the merge output before any resolution.
