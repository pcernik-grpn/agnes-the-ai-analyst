"""The four surfaces the admin cleanup retired: Studio (with its suggestions
queue), news, the maintained-digests admin page, and contribute-a-skill.

All four are HIDDEN, not deleted. The pages, templates and APIs are intact and
...
SURFACES = {
    "News": ("AGNES_NEWS_ENABLED", "/admin/news"),
...
def test_sidebar_row_is_absent_by_default(seeded_app, label):
    ...
    assert f">{label}</a>" not in resp.text
...
def test_sidebar_row_comes_back_with_its_flag(seeded_app, monkeypatch, label):
    ...
    for other, (_, _page) in SURFACES.items():
        if other != label:
            assert f">{other}</a>" not in resp.text, f"{env_var} also exposed {other!r}"
