...
This file covers:
  ...
  - the one-time backfill migration (`src.db._v122_to_v123`);
...
def test_backfill_migrates_name_keyed_rows_to_registry_id(tmp_path):
    from src.db import _ensure_schema, _v122_to_v123
...
def test_backfill_is_idempotent(tmp_path):
    ...
    _v122_to_v123(conn)
    _v122_to_v123(conn)  # re-run must not raise or change anything
...
def test_backfill_skips_collision_with_existing_row(tmp_path):
    ...
    _v122_to_v123(conn)  # must not raise
