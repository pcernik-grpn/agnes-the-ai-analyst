...
# ---------------------------------------------------------------------------
# adopt_source — a writer claiming rows it made before it stamped them
#
# The backends legitimately differ here and the tests say so rather than
# skipping: `resource_grants.source` arrived in Alembic 0096, which is
# Postgres-only under the A3 ratchet, so the DuckDB app-state backend has no
# such column. Postgres stamps; DuckDB accepts and drops, exactly as
# `create(source=...)` already does.
# ---------------------------------------------------------------------------


def test_adopt_source_stamps_a_grant_that_has_none(rbac_repos):
    """The SharePoint ACL sync wrote its grants for a long time without
    recording a source. /admin/access reads `source` to decide whether a
    revoke can hold, so those rows were filed under "change here" and drew a
    Revoke that the next sync silently undid. This is how the sync adopts
    them on its next run instead of leaving every existing instance wrong.
    """
    repos, _, backend = rbac_repos
    groups, grants = repos["groups"], repos["grants"]
    grp = groups.create(name="mirrored", created_by="admin@x.com")
    gid = grants.create(
        group_id=grp["id"],
        resource_type="marketplace_plugin",
        resource_id="sp/unstamped",
        assigned_by="system:sharepoint-acl-sync",
    )
    wrote = grants.adopt_source(gid, "sharepoint_acl_sync")
    if backend == "pg":
        assert wrote is True
        assert grants.get(gid)["source"] == "sharepoint_acl_sync"
    else:
        # No column to write, so nothing was written — and saying False is
        # what lets the caller log honestly instead of claiming a repair.
        assert wrote is False
        assert "source" not in grants.get(gid)


def test_adopt_source_never_overwrites_a_source_someone_else_recorded(rbac_repos):
    """The one thing it must not do. `source` decides whether the page
    offers a revoke; relabelling another writer's grant would move a row
    between "you can change this" and "you cannot" behind the admin's back.
    """
    repos, _, backend = rbac_repos
    if backend != "pg":
        pytest.skip("no `source` column on the frozen DuckDB app-state ladder")
    groups, grants = repos["groups"], repos["grants"]
    grp = groups.create(name="mixed", created_by="admin@x.com")
    gid = grants.create(
        group_id=grp["id"],
        resource_type="marketplace_plugin",
        resource_id="sp/already-stamped",
        assigned_by="admin@x.com",
        source="sharepoint_wizard",
    )
    assert grants.adopt_source(gid, "sharepoint_acl_sync") is False
    assert grants.get(gid)["source"] == "sharepoint_wizard"


def test_adopt_source_on_a_missing_grant_is_false_not_an_error(rbac_repos):
    """It runs inside a sync loop over rows read a moment earlier; a grant
    deleted in between must not take the run down. True on both backends."""
    repos, _, _ = rbac_repos
    assert repos["grants"].adopt_source("no_such_grant", "sharepoint_acl_sync") is False
