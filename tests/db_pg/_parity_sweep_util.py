...
# Re-exported, not defined here (issue #1658): the same repair is also needed
# by ``tests/conftest.py``'s ``duckdb_backend_pinned`` fixture, which protects
# the session-shared app used by plain (non-``db_pg``) DuckDB-backend tests
# from this same reload. See ``tests/_backend_pin.py`` for the full docstring
# and the regression test this mechanism backs.
from tests._backend_pin import reregister_requires_pg_handler as _reregister_requires_pg_handler
