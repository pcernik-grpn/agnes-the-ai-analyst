...
def _batches(items: Iterable[_BatchItem]) -> Iterator[list[_BatchItem]]:
    ...
    there is no smaller unit to split it into. The key half of each item is
    untyped (:data:`_BatchItem`) purely so the aux "late feedback" sweep —
    which has no keyset position, only a bare record — can reuse this with
    ``None`` keys rather than duplicating the split logic.
    """
...
def run_conversation_export_once(
    *,
    ...
    return {"sent": total_sent, "batches": batches_sent, "batches_failed": batches_failed, "refreshed": refreshed}
