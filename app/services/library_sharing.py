...
Store entities (skills, plugins, agent templates) were deliberately absent
here, on the grounds that a grant row on one would be dead mechanics: an
approved entity is visible to every authenticated user, so nothing read the
row. That premise no longer holds — a ``store_entity`` grant is now read by
``_enforce_visibility``, the browse listing, the install gate and the serve
chokepoint, so a PRIVATE entity reaches exactly the groups it was shared with
and no one else. Sharing one is therefore an ordinary owner-scoped grant, with
the same containment rules as every other type.

What is still missing is a different thing, and remains a real gap:

  TODO — an owner cannot change their own store entity's ACCESS tier. A skill
  kept Private has no path to Everyone from any surface: ``access`` is
  accept-only at create (``POST /api/store/entities`` and its ``from-markdown``
  sibling), and ``PUT /api/store/entities/{id}`` carries no ``access`` field.
  Sharing it with a group is now possible; publishing it to the whole instance
  ...
  Postgres repo siblings in one change; REST × CLI × MCP coverage.
...
def _store_entity_owner(resource_id: str) -> Optional[str]:
    """A skill / plugin / agent template's owner is its author.

    Store entities were excluded from this module on the grounds that a grant
    row on one would be dead mechanics — an approved entity is visible to
    every authenticated user, so nothing read the row. That was true, and it
    is what made "share my private skill with my team" impossible: the only
    choices were you-only and the entire instance.

    The grant is now read — ``_enforce_visibility``, the browse listing, the
    install gate and the serve chokepoint all honour it — so a private entity
    reaches exactly the groups it was shared with. The exclusion went with the
    premise.
    """
    from src.repositories import store_entities_repo

    row = store_entities_repo().get(resource_id)
    return row.get("owner_user_id") if row else None
...
_OWNER_RESOLVERS[ResourceType.STORE_ENTITY.value] = _store_entity_owner
...
SHAREABLE_TYPES: Dict[str, str] = {
    ...
    ResourceType.STORE_ENTITY.value: "item",
}
