...
- A turn record now says when its turn closed, so a completion that finishes
  ...
  from a turn the system lost. The cut stream a cancel produces is not
  counted as an error, but only when the cancel really produced one — a
  cancel that lands while a tool runs cuts no stream, and a real failure
  earlier in the same session survives either way.
