...
**Level 2 — containers.** The separately deployable and runnable pieces and the
stores they keep state in: one image running the `api`, `gateway` and `worker`
roles, the four sidecars, the containers Agnes spawns per session, and — outside
the server boundary — the analyst's own laptop.

[![Agnes containers](docs/diagrams/agnes-c4-container.svg)](docs/diagrams/agnes-c4-container.svg)

**Level 3 — components.** One diagram per subsystem worth zooming into. The
application container, where a request becomes an authorized read:

[![Agnes application components](docs/diagrams/agnes-c4-component-app.svg)](docs/diagrams/agnes-c4-component-app.svg)

The data plane — one contract from every source, and the path a parquet takes to
a laptop:

[![Agnes data-plane components](docs/diagrams/agnes-c4-component-data.svg)](docs/diagrams/agnes-c4-component-data.svg)

The agent runtime — how one turn runs, and the two fail-closed chokepoints in
front of the model:
...
Notation, since a C4 drawing has to be self-describing: a name, a bracketed type
tag naming the element's kind and technology, then what it does. Dashed means
outside the boundary in focus; azure is the data path and royal blue the
agent/LLM path, matching the layered figures above.

---

All eight figures are generated by
