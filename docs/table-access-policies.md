...
| `policy_mapping_empty` | 500, `POST /api/query` only | a `policy_mapping` table the policy joins has zero (or never-synced) rows — named alongside the policied table (`table`, `mapping_table`, `note`, plus `last_sync`: the mapping table's last successful sync timestamp, `null` if it never synced), same underlying check and wording as `GET /api/me/effective-access`'s `reason: "mapping_empty"` (`src.access_policy.raise_if_policy_mapping_empty`, the one implementation both call) |
