...
| `studio.enabled` | first-boot-seed (was bootstrap-env before D1) | `/admin/server-config` (after day 1); `var.studio_enabled` (day-1 seed only, module-wide — defaults to `false` now, and seeds only when explicitly `true`, since the app default flipped) | `AGNES_STUDIO_ENABLED` env (if hand-set) > `instance.yaml` > default |
...
| Inline `<svg>` brand MARK for slots a lockup doesn't fit (rail collapsed to 56px) | `AGNES_INSTANCE_LOGO_MARK_SVG` | `instance.logo_mark_svg` | `""` (initial derived from the brand) | `get_instance_logo_mark_svg()` |
...
| Expose the authoring Studio (`/admin/studio*` incl. the admin moderation queue, plus the public suggestion API). `false` hides the nav/palette entries, redirects the routes home, and 403s the suggestion API. **Default flipped to `false`** when the admin cleanup retired the surface — the Library builders (`/library` → "+ New") do the same authoring jobs; the pages are intact, so `true` restores all of it | `AGNES_STUDIO_ENABLED` | `studio.enabled` | `false` | `get_studio_enabled()` |
| Expose in-product news: the `/admin/news` editor, the `/news` reader, the `/home` "What's new" strip and their rail/palette entries. UI only — `/api/admin/news/*` keeps serving and a published version is preserved, so re-enabling restores the surface with its content | `AGNES_NEWS_ENABLED` | `features.news_enabled` | `false` | `get_news_enabled()` |
| Expose the maintained-digests admin PAGE (`/admin/knowledge-digests`) and its nav row. Deliberately narrow: `/api/admin/knowledge-digests/*`, `agnes admin digest`, the digest scheduler job and `agnes pull`'s digest delivery are untouched, so an instance keeps running digests headlessly while the page is hidden | `AGNES_KNOWLEDGE_DIGESTS_ENABLED` | `features.knowledge_digests_enabled` | `false` | `get_knowledge_digests_ui_enabled()` |
| Expose the paste-a-SKILL.md publish page (`/admin/contribute-skill`), the landing target for an external "Load skill to Agnes" button. Both POST handlers carry the gate, so a stale external button gets a redirect home rather than a silent publish; the Library's skill builder is the supported path | `AGNES_CONTRIBUTE_SKILL_ENABLED` | `features.contribute_skill_enabled` | `false` | `get_contribute_skill_enabled()` |
| Expose the Moderation & Trust hub (`/admin/store`) and its nav + palette entries. Its three zones each have a nearer door — Submissions is its own nav row, curation is `/admin/marketplaces`, verification has `store.verification_enabled` — so the page was a landing spot for links already in the column. UI only: the store APIs and `/api/admin/share-requests*` keep serving | `AGNES_STORE_MODERATION_ENABLED` | `features.store_moderation_enabled` | `false` | `get_store_moderation_enabled()` |
| Expose the unattended ANALYST onboarding layer: the four guided coach-mark tours (the welcome walkthrough on `/chat`, the `/agents` and Connect cards, the skill-builder mark) and the analyst checklist card in the rail foot, with its popover, its replay control and the profile menu's "Start over onboarding" entry. **On by default** — a kill switch for a surface every instance already has, so an upgrade changes nothing. The ADMIN setup chain is a separate card and stays. UI only: `/api/chat/journey` keeps serving and keeps recording steps, so re-enabling resumes every user where they were. The chat's greeting and the empty-Stack data-package recommendation are outside the switch | `AGNES_ONBOARDING_ENABLED` | `features.onboarding_enabled` | `true` | `get_onboarding_enabled()` |
