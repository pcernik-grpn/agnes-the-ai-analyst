...
MCP tool calls take the same round-trip. The file hook runs for them first —
its `deny` is an operator policy decision and is enforced as one — and a call
it does not deny is then routed by the tool's declared `readOnlyHint`: a
read-only tool runs unasked, anything else raises the card, carrying the tool
name and its arguments. The runner evaluates a name allowlist copied from
those annotations (`_READ_ONLY_AGNES_MCP_TOOLS`, pinned to them by a test)
rather than reading annotations at tool time, and the allowlist is keyed on
`(server, tool)` so another MCP server cannot borrow an Agnes verdict. The
equivalent CLI mutations are `ask`-flagged in the bundled hook so the Bash
route raises the same card; what that does NOT cover is a `curl` straight at
the in-sandbox relay, or the agent rewriting the hook file — see the
limitations list below, and `SECURITY.md` for why the hook is
defense-in-depth and never the boundary.
...
- **The approval gate matches `Bash` tool calls and MCP tool calls, under the `docker` provider only.** MCP tools follow the `readOnlyHint` annotation each tool declares, but the rule the runner evaluates at tool time is a name allowlist copied from those annotations and pinned to them by a test (`app/chat/runner.py::_READ_ONLY_AGNES_MCP_TOOLS`) — the runner cannot read annotations from a server it does not import. A listed tool runs unasked; anything else — including a tool the allowlist does not know (a per-caller passthrough tool, or one from a workspace-configured MCP server) — takes the same approve/deny round-trip. The workspace file hook runs first for MCP calls too, so an operator `deny` is enforced rather than downgraded to a card; its `allow` is not a decision (the bundled hook allows every non-Bash tool by default). The `kai-agent` provider does not use this gate at all — the engine raises its own `tool-approval-request` events, which land on the same web card. Built-in non-Bash tools (`Read`/`Write`/`Edit`/`WebFetch`) are still not gated: the bundled workspace hook returns `allow` for every non-Bash tool, so no policy is lost as shipped, and an operator override that adds `ask` rules for them would find them inert in cloud chat until the SDK hook matchers (`app/chat/runner.py::_build_pretool_matchers`) are widened — a deliberate scope choice (gating every `Read`/`Write` through a per-call file-hook subprocess adds real latency).
- **The gate is fail-closed only where it is armed.** On the no-timeout SDK path (a `HookMatcher` that takes no `timeout`) both matchers stay registered with the gate disabled, so ask-flagged commands and mutating MCP tools are DENIED rather than run unasked, and an internal error inside the gate denies too. But on an SDK with no PreToolUse hook support at all (`ClaudeAgentOptions` without `hooks`, or no `HookMatcher`) there is nothing to register: the gate logs loudly and **everything runs unasked**, Bash and MCP alike. The sandbox image's SDK is outside the wheel's pin, so treat that log line as an operational alarm.
- **The gate covers tool calls, not every route to the same mutation.** The CLI mutations that mirror gated MCP tools are `ask`-flagged in the bundled workspace hook (`agnes admin grant|group|user`, `agnes app create|deploy|stop|delete|draft …|git-credential|set-description`), so they raise the same card through Bash. Two gaps remain by construction: the agent can reach the in-sandbox relay directly with `curl` and its scoped ticket, bypassing the CLI and its hook entirely; and the hook is a file inside the workspace the agent can rewrite. Both are the same defense-in-depth caveat `SECURITY.md` already states — the enforcing boundary is the sandbox and the server's own RBAC, never the hook.
