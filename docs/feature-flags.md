...
| `extraction` | `extraction.enabled` | `AGNES_EXTRACTION_ENABLED` | `false` | no — needs the `extraction` optional dependency extra installed AND a worker process polling the `extraction` lane (the `extraction-worker` Compose profile) | Document extraction (spec §7.5 "Extraction inside Agnes") as its own worker lane — gates the `corpus-extraction` job kind's handler, which runs the built-in crawl → convert → anonymize → ingest pipeline (`connectors/sharepoint/crawler.py`) in-process, bounded by `extraction.timeout_s`. Off by default. Turning this on is NOT enough by itself to run the worker: the `extraction-worker` Compose service sets `AGNES_ROLE=worker`, a role split, so the deployment also needs the multi-process prerequisites (Postgres app-state, explicit `JWT_SECRET_KEY`/`SESSION_SECRET`, `coordination.backend: redis`) — see [`DEPLOYMENT.md#multi-process`](DEPLOYMENT.md#multi-process). |
