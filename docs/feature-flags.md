...
| `acl_sweep_interval_days` | `acl_sync.sweep_interval_days` | `AGNES_ACL_SWEEP_INTERVAL_DAYS` | `7` | yes | Days between full `sharepoint-subtree-sweep` passes (broken-inheritance folder detection, spec §3(b)) for one connection's mirrored scopes. The scheduler row itself already fires weekly (native cron); this is a per-connection self-guard against a restart-refire, not the primary cadence. |
