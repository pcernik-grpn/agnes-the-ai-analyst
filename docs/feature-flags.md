...
- `editable` / `lock_reason` — whether the admin UI offers a write path for
  ...
  backend running. `extraction` (a static section, not a switch — see its
  own note below) is editable (reversed from its original deploy-time-only
  stance) but is still not a simple case: a deploy-time env var can pin
  either of its two producer leaves ahead of a web save, and the panel
  refuses a write to a pinned leaf instead of accepting one the runtime
  would never read.
...
| `store_moderation` | `features.store_moderation_enabled` | `AGNES_STORE_MODERATION_ENABLED` | `false` | yes | The Moderation & Trust hub (`/admin/store`), its nav row, its palette row and the `g v` shortcut. Off by default: each of its three zones has a nearer door (Submissions' own nav row, `/admin/marketplaces`, and verification's own `store.verification_enabled`), so the hub was a landing page for links the column already carries. Hides UI only — the store APIs and `/api/admin/share-requests*` keep serving, so a queued agent share stays decidable by API. |
| `onboarding` | `features.onboarding_enabled` | `AGNES_ONBOARDING_ENABLED` | `true` | yes | The unattended ANALYST onboarding layer: all four guided coach-mark tours (the welcome walkthrough on `/chat`, the `/agents` and Connect cards, the skill-builder mark) plus the analyst checklist card in the rail foot, its popover, its replay control and the profile menu's "Start over onboarding" entry. **On by default** — a kill switch for an existing surface, not a new feature, so an upgrade changes nothing. Enforced in one place per layer: `tour.js`'s three exported entry points (`launchTour` / `autoLaunchTour` / `resumePendingTour`) and `chat_onboarding.js`'s `renderJourneyPanel` / `mountJourneyPanel`, both reading `window._agOnboardingEnabled` as stamped by `_app_scripts.html` — undefined means ON, only an explicit `false` disables. Hides UI only: `/api/chat/journey` keeps serving and keeps recording steps, so turning it back on resumes every user where they were. NOT covered, deliberately: the ADMIN setup chain (a different rail card — an operator keeps their own setup progress), the chat's greeting, and the empty-Stack question that recommends data packages. |
...
| `mcp_query_param_token` | `mcp.allow_query_param_token` | `AGNES_MCP_ALLOW_QUERY_PARAM_TOKEN` | `false` | yes | **Off by default since the #1656 audit follow-up** (was grandfathered on). Accepts the MCP bearer token as `?token=` on SSE GET for a header-incapable client; the token then appears in every request log when used (CWE-598). With it off, a `?token=`-only request is refused exactly like a missing credential (401 `no_token`). Turn on only for a documented client that genuinely cannot send the `Authorization` header — every connection snippet Agnes hands out is header-based already. Agnes's flag parser treats every unrecognized string as truthy (only `false`/`off`/`no`/`0` disable it), so verify the flag's `effective` value via `/admin/server-config` after any explicit change. |
...
| `microsoft_group_sync` | `auth.microsoft.group_sync_enabled` | `AGNES_MICROSOFT_GROUP_SYNC_ENABLED` | `false` | yes | Mirror the signed-in user's Entra ID group memberships (Microsoft Graph `GET /me/transitiveMemberOf/microsoft.graph.group`) into `user_group_members` (`source='microsoft_sync'`) on every Microsoft sign-in — the group-sync counterpart of `google_sync`. Groups are keyed `entra:<object-id>`, the SAME key the SharePoint ACL mirror uses for the same Entra group. New feature — off by default: turning it on also widens the OAuth consent scope requested at `/auth/microsoft/login` to the delegated Graph permission `GroupMember.Read.All`, which needs its own admin-consent grant in the Entra app registration (see `docs/auth-microsoft-oauth.md`) and only takes effect after a restart — the sync gate itself is read live, so a stale token scope degrades to a fail-soft no-op rather than a login failure. `AGNES_MICROSOFT_GROUP_PREFIX` (env-only, no `instance.yaml` key — mirrors `AGNES_GOOGLE_GROUP_PREFIX`) narrows which fetched groups are mirrored / required to sign in. |
...
| `sharepoint` | `sharepoint.enabled` | `AGNES_SHAREPOINT_ENABLED` | `false` | yes | The WHOLE SharePoint connector, one flag (2026-09-01 flag consolidation — see the migration note below): the connect wizard and every `/api/admin/sharepoint/*` route (`409 feature_disabled` when off), document crawling/extraction (`corpus-extraction` job kind, the `extraction.schedule` sweep, the Microsoft Graph change-notification receiver at `POST /api/webhooks/sharepoint/{connection_id}`, `404` when off), and source-ACL mirroring (`sharepoint-acl-sync`/`sharepoint-subtree-sweep` jobs, permission zones, the fact/upload ingest gate). Per-scope choices (`access_mode='mirrored'`, `anonymize`) still live in the wizard — this flag only decides whether the connector exists at all. New feature — off by default. Turning extraction on is NOT enough by itself to run the worker: a process must actually poll the `extraction` lane (`AGNES_WORKER_LANES`, e.g. the `extraction-worker` Compose profile), which sets `AGNES_ROLE=worker`, a role split, so the deployment also needs the multi-process prerequisites (Postgres app-state, explicit `JWT_SECRET_KEY`/`SESSION_SECRET`, `coordination.backend: redis`) — see [`DEPLOYMENT.md#multi-process`](DEPLOYMENT.md#multi-process). `extraction.schedule` and `extraction.timeout_s` are editable from `/admin/server-config` in the separate (switch-less) `extraction` section; neither carries a deploy-time env override — the `AGNES_EXTRACTION_PRODUCER_*` locks were removed together with the external producer mode (the built-in in-process pipeline is the only pipeline). |
| `extraction_facts` | `extraction.facts.enabled` | `AGNES_EXTRACTION_FACTS_ENABLED` | `false` | yes | The LLM stage of the built-in extraction pipeline (`connectors/sharepoint/facts_extraction.py`) — after a successful crawl, read each newly ingested prose document and turn it into evidence-carrying graph facts, shipped through the same `POST /api/facts/ingest` contract an external producer would use. A **cost** switch, which is why it is off by default and separate from `sharepoint.enabled`: this is the only stage that spends model tokens per document — a live 187-document pass measured $0.0542/document on the default Haiku-class model (`config/instance.yaml.example`'s `facts` block, `docs/superpowers/specs/2026-09-02-fact-extraction-cost-levers.md`), roughly 3× on Sonnet; re-measure rather than trust either figure blindly. Needs `facts.enabled` too — with the fact-graph surface off the pass is skipped rather than writing claims no endpoint would serve. Spreadsheets and CSVs are sent like any other document (their converted markdown tables carry facts too) — dense/tabular text is charged a tighter token ratio and, if too large even at the token budget, skipped rather than truncated to a meaningless head (`docs_skipped_too_large_tabular`); a binary/decode-garbage document (a failed conversion) is skipped outright (`docs_skipped_garbled_text`). A re-run re-extracts only documents whose content, model, or effective prompt changed. Companion keys in the same section: `extraction.facts.concurrency` (documents extracted in parallel, default 3, clamped to [1, 16]; 1 is exactly sequential) and `extraction.facts.model` (optional per-stage model override). The prompt itself is editable at `/api/admin/prompts/facts-extraction`; the ontology comes from the semantic-model store. |
| `acl_guarantee_mode` | `acl_sync.guarantee_mode` | `AGNES_ACL_GUARANTEE_MODE` | `must_not` | yes | Cross-audience-leak posture (design Q7). `must_not` (default) = fail closed: broken-inheritance subtrees are always excluded, mirrored grants are suspended past `acl_sync.max_stale_hours`, and untagged claims in an audience-tiered scope are admin-only. `should_not` = best effort: advisory overrides allowed, stale grants persist with warnings, untagged claims stay unrestricted within their collection. |
| `acl_max_stale_hours` | `acl_sync.max_stale_hours` | `AGNES_ACL_MAX_STALE_HOURS` | `72` | yes | `must_not` mode only: hours a failed ACL sync may leave mirrored grants standing before they are suspended (deleted until the next successful sync rewrites them). |
| `acl_sync_interval_hours` | `acl_sync.interval_hours` | `AGNES_ACL_SYNC_INTERVAL_HOURS` | `4` | no — read by the scheduler sidecar at startup | Hours between SharePoint ACL sync runs (scope + zone root permission re-reads). The source-side revocation window is at most this long. |
| `acl_sweep_interval_days` | `acl_sync.sweep_interval_days` | `AGNES_ACL_SWEEP_INTERVAL_DAYS` | `1` | yes | Days between full `sharepoint-subtree-sweep` passes (broken-inheritance folder detection, spec §3(b)) for one connection's mirrored scopes. The scheduler row itself already fires daily (native cron); this is a per-connection self-guard against a restart-refire, not the primary cadence. Daily by default so a newly broken-inheritance subtree is detected within ~a day (MUST NOT posture); raise it if the Graph probe budget on a very large tenant becomes a problem. |

> **Migration note (2026-09-01):** four separate SharePoint flags —
> `extraction.enabled` (`AGNES_EXTRACTION_ENABLED`),
> `extraction_webhook.enabled` (`AGNES_EXTRACTION_WEBHOOK_ENABLED`),
> `acl_mirroring.enabled` (`AGNES_ACL_MIRRORING_ENABLED`), and
> `acl_sync.zones_enabled` (`AGNES_ACL_ZONES_ENABLED`) — collapsed into the
> single `sharepoint.enabled` (`AGNES_SHAREPOINT_ENABLED`) switch above.
> None of the four old config keys or env vars are read anymore. An
> instance that had `extraction.enabled: true` (or any of the other three)
> must set `sharepoint.enabled: true` (or `AGNES_SHAREPOINT_ENABLED=1`) on
> upgrade, or the whole connector — including extraction — goes dark.
> Permission zones (previously the opt-in `acl_zones` switch) are no longer
> a separate choice: whenever the connector is on, a broken-inheritance
> folder is always promoted to its own permission zone rather than merely
> excluded.
