...
- `editable` / `lock_reason` — whether the admin UI offers a write path for
  ...
  backend running. `extraction` is editable (reversed from its original
  deploy-time-only stance — see its own row below) but is still not a
  simple case: a deploy-time env var can pin any of three of its leaves
  ahead of a web save, and the panel refuses a write to a pinned leaf
  instead of accepting one the runtime would never read.
...
| `extraction` | `extraction.enabled` | `AGNES_EXTRACTION_ENABLED` | `false` | yes (env-lockable — see below) | Document extraction (spec §7.5 "Extraction inside Agnes (later)") as its own worker lane — gates the `corpus-extraction` job kind's handler, which shells out to the operator-configured producer command/module. New feature — off by default. Turning this on is NOT enough by itself to run the worker: a process must actually poll the `extraction` lane (`AGNES_WORKER_LANES`, e.g. the `extraction-worker` Compose profile), which sets `AGNES_ROLE=worker`, a role split, so the deployment also needs the multi-process prerequisites (Postgres app-state, explicit `JWT_SECRET_KEY`/`SESSION_SECRET`, `coordination.backend: redis`) — see [`DEPLOYMENT.md#multi-process`](DEPLOYMENT.md#multi-process). `extraction.producer.command`/`.module`/`.env_passthrough`, `extraction.schedule` and `extraction.timeout_s` are ALSO editable from `/admin/server-config` alongside this switch (reversing the original deploy-time-only stance). Three leaves carry a deploy-time env override that always wins over a web save — `enabled` (`AGNES_EXTRACTION_ENABLED`), `producer.command` (`AGNES_EXTRACTION_PRODUCER_COMMAND`), `producer.module` (`AGNES_EXTRACTION_PRODUCER_MODULE`), the same three the Terraform customer-instance module renders (docs/DEPLOYMENT.md#multi-process). `GET /api/admin/server-config`'s `known_fields.extraction` marks a currently-pinned leaf `env_locked: true` and its `sections.extraction` shows the actual env-resolved value; `POST` refuses a write to a pinned leaf with `409 field_locked_by_deployment` rather than silently storing a value the runtime would never read. `schedule`/`timeout_s`/`env_passthrough` carry no such env override and are always plain writable fields. |
| `acl_mirroring` | `acl_mirroring.enabled` | `AGNES_ACL_MIRRORING_ENABLED` | `false` | yes | SharePoint ACL mirroring: the `sharepoint-acl-sync` job, per-scope `access_mode='mirrored'`, and the admin sync-now endpoint. OFF by default — turning it on changes nothing until a scope opts into mirroring. |
...
| `acl_zones` | `acl_sync.zones_enabled` | `AGNES_ACL_ZONES_ENABLED` | `false` | yes | Promote broken-inheritance SharePoint subtrees to their own collections with their own mirrored ACLs (permission zones) instead of excluding them from the crawl entirely. Requires `acl_mirroring`. |
