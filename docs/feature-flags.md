...
| `sharepoint` | `sharepoint.enabled` | `AGNES_SHAREPOINT_ENABLED` | `false` | yes | The WHOLE SharePoint connector, one flag (2026-09-01 flag consolidation — see the migration note below): the connect wizard and every `/api/admin/sharepoint/*` route (`409 feature_disabled` when off), document crawling/extraction (`corpus-extraction` job kind, the `extraction.schedule` sweep, the Microsoft Graph change-notification receiver at `POST /api/webhooks/sharepoint/{connection_id}`, `404` when off), and source-ACL mirroring (`sharepoint-acl-sync`/`sharepoint-subtree-sweep` jobs, permission zones, the fact/upload ingest gate). Per-scope choices (`access_mode='mirrored'`, `anonymize`) still live in the wizard — this flag only decides whether the connector exists at all. New feature — off by default. Turning extraction on is NOT enough by itself to run the worker: a process must actually poll the `extraction` lane (`AGNES_WORKER_LANES`, e.g. the `extraction-worker` Compose profile), which sets `AGNES_ROLE=worker`, a role split, so the deployment also needs the multi-process prerequisites (Postgres app-state, explicit `JWT_SECRET_KEY`/`SESSION_SECRET`, `coordination.backend: redis`) — see [`DEPLOYMENT.md#multi-process`](DEPLOYMENT.md#multi-process). `extraction.producer.command`/`.module`/`.env_passthrough`, `extraction.schedule` and `extraction.timeout_s` are editable from `/admin/server-config` in the separate (switch-less) `extraction` section. Two of those leaves carry a deploy-time env override that always wins over a web save — `producer.command` (`AGNES_EXTRACTION_PRODUCER_COMMAND`), `producer.module` (`AGNES_EXTRACTION_PRODUCER_MODULE`), the same two the Terraform customer-instance module renders (docs/DEPLOYMENT.md#multi-process). `GET /api/admin/server-config`'s `known_fields.extraction` marks a currently-pinned leaf `env_locked: true` and its `sections.extraction` shows the actual env-resolved value; `POST` refuses a write to a pinned leaf with `409 field_locked_by_deployment` rather than silently storing a value the runtime would never read. `schedule`/`timeout_s`/`env_passthrough` carry no such env override and are always plain writable fields. |
