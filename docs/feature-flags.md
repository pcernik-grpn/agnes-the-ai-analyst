...
| `chat_provider` | `chat.provider` | `AGNES_CHAT_PROVIDER` | `e2b` | yes (applies after restart) | Which engine runs web/Slack chat sessions: `e2b`, `docker`, or `kai-agent` (the embedded turn engine — docs/cloud-chat.md). Resolved by `load_chat_config` (env > instance.yaml > default) at boot; every provider rides deployment-provisioned backing (E2B key/template, apps-runner sidecar, or the engine sidecar + `KAI_HOST_JWT_SECRET`), and the boot gates refuse a provider whose backing is absent, loudly. Pin it in infrastructure via the customer-instance module's per-VM `chat_provider` field so a fresh data disk boots into the right engine. |
...
| `chat_bootstrap_marketplace` | `chat.bootstrap_marketplace` | `AGNES_CHAT_BOOTSTRAP_MARKETPLACE` | `true` | yes (applies after restart) | Deliver the caller's RBAC-filtered marketplace skills into chat sessions, so a skill in their stack is invokable as `/<skill-name>`. Agnes materializes the skill directories server-side: into the per-user chat workspace for `e2b`/`docker` (`app/chat/workdir.py`), into the workspace tarball the engine unpacks for `kai-agent` (`app/api/kai.py`) — one mechanism, two placements, one slash token. ON by default since 0.87.1: with it off the composer's slash menu still listed those skills while nothing delivered them, so picking one answered "Unknown command" (#1552). Turning it off now also removes them from the menu — the menu never offers what nothing delivers. |
