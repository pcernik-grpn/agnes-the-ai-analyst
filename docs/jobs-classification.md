...
| `sharepoint-subtree-sweep` | `POST /api/jobs` (`kind=sharepoint-subtree-sweep`) | queued | 2026-08-30 plan, Task 7 — walks each mirrored scope's folder tree to detect and exclude broken-inheritance subtrees (`connectors/sharepoint/acl_sync.py::run_subtree_sweep`) — a full probe pass is multi-hour on a large library (spec §6.2), so LIGHT lane with a much longer lease (`AGNES_SP_SWEEP_LEASE_S`, default 4h) and no automatic retry. `cron 0 7 * * 1` (weekly, native cron — same grammar as `store-lint-audit`), not nightly. Enqueued unconditionally; the handler no-ops when `acl_mirroring.enabled` is false, and each connection additionally self-guards against a restart-refire via its own `acl_sweep_last_full`/`acl_sync.sweep_interval_days` check. |
