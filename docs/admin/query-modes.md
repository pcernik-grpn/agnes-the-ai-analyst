...
- **Filtered export** — `source_query` holds a **JSON filter spec**, never SQL: `{"where_filters": [{"column": "date", "operator": "ge", "values": ["{{last_3_months}}"]}]}`. Accepted keys are `where_filters`, `columns`, `changed_since`, `changed_until`, `limit` and `file_type` (plus the `fileType` wire-name alias), and a `where_filters` entry takes `column`, `operator` and `values` and nothing else; operators are `eq, ne, gt, ge, lt, le`, and the date placeholders resolve at sync time. **Any other key is refused** — at registration, at update, in the advanced JSON editor and at sync time. An unknown key used to be dropped silently, which turned a misspelled row filter (`where_filter`, `whereFilters`, `changedSince`) into a full-table export the admin never asked for. A `source_query` starting with `SELECT`/`WITH` is refused at registration and at update time — for DuckDB-SQL pulls use `query_mode: local` (Direct extract) instead.
