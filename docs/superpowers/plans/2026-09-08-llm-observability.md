...
| `migrations/versions/0117_llm_observability.py` (new) | 6 | `llm_calls`, `chat_message_feedback`, `chat_messages.turn_id`, `agent_memories.source_turn_id/source_message_id` |
...
**Files:**
- Create: `migrations/versions/0117_llm_observability.py`
...
- `app/chat/persistence.py::append_message(..., turn_id=None)`: forward to `_messages_pg` when present; on the DuckDB path do not write it (comment: PG-only column, migration 0117 — the same accept-and-drop as the cache columns).
...
```bash
git add migrations/versions/0117_llm_observability.py src/models/llm_observability.py src/models/chat.py src/models/agents.py \
...
- Create: migration `0118_export_watermarks.py` — table `export_watermarks (name text pk, watermark timestamptz, updated_at timestamptz)`; model; PG-only repo `export_watermarks_pg.py`; registry entry
...
| `app/worker/kinds_conversation_export.py`, `migrations/versions/0118_export_watermarks.py`, `src/repositories/export_watermarks_pg.py` (new) | 11 | corpus export, push |
