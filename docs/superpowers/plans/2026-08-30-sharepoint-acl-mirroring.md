...
- ~~`_run_corpus_extraction` env handoff (`AGNES_SP_EXCLUDED_SUBTREE_IDS`)~~ — **superseded 2026-08-31**: external-producer mode was removed (builtin-only decision); the built-in crawler honors `excluded_subtrees` in-process, and zone routing lands with TCRD-284.
