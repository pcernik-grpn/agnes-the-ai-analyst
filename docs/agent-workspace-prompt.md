...
| `facts.enabled` | bool | `True` iff the `facts` feature switch is on for this instance. Gates the "Facts — entity and relationship questions" section that steers who/what/relationship/aggregation questions to `fact_search`/`fact_neighbors`/`fact_edges`/`fact_claims` before SQL or document search, plus one sentence in "Documents — find the folder before you search it" pointing at the graph as the fastest route to a client's folder |
