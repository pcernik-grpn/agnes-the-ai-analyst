...
- **Treat "ready for review" as "ready to merge".** Take a PR out of draft (`gh pr ready <N>`) only when you would be comfortable with it landing on `main` as-is: the change is complete, the `[Unreleased]` CHANGELOG bullet is in, the test lane you owe is green, and any review feedback you were waiting on is addressed. **Anything not ready to merge stays a draft** (`gh pr create --draft`) — work in progress, a branch opened early for CI, a PR blocked on a question, one opened purely for discussion. Leaving draft is the merge decision; make it deliberately.
- **Leaving draft does not merge the PR by itself.** Nothing here merges a ready PR on your behalf: `allow_auto_merge` only means the button exists, and a green non-draft PR sits untouched until someone calls merge. Enable auto-merge in the same breath as marking it ready — `gh pr merge <N> --squash --auto --delete-branch`, step 6 of the feature-PR recipe in [`docs/RELEASING.md`](docs/RELEASING.md) — so it lands as soon as the required checks pass.
- The daily release-cut PR is the exception: `.github/workflows/daily-cut.yml` opens it non-draft and deliberately leaves auto-merge off, because a human merges it (see [`docs/RELEASING.md`](docs/RELEASING.md)).
