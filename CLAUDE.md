...
# One full lane per machine at a time. Parallel builder agents on one host run
# `--lane impacted` only; the host's single `fast` lane runs once, before the
# push. Do not re-run a lane because `main` moved — CI tests that head anyway.
...
Admin RBAC for auto-sync flows through data packages (per-table `resource_grants` no longer surface tables in analyst manifests): the admin adds `query_mode IN ('local', 'materialized')` tables to a data package and grants the package to one of the analyst's groups; how the package then joins the analyst's stack depends on `features.stack_auto_membership` (default **on** since Wave 0, 2026-08). Auto-membership (the default): every granted package — required or available — is in the stack immediately, and a subscription (`agnes stack add data_package <id>`) only controls whether `agnes pull` keeps a local copy (required packages are always local). Classic (the explicit `false` opt-out, fully supported and it always wins): membership itself is `required ∪ subscribed` — an available grant surfaces only once the analyst subscribes (`agnes stack browse` / `agnes stack add`). Either way its tables land in their manifest for `agnes pull` to download. No per-user sync config; the admin layer plus the user's stack are the source of truth.
...
- **Changelog discipline.** Every PR that changes user-visible behavior MUST add a CHANGELOG **fragment** in the same PR: one file `changelog.d/<slug>.md` holding `### Added|Changed|Fixed|Removed|Internal` headings and bullets, `**BREAKING**` prefix for breaking changes (format: `changelog.d/README.md`). No follow-ups. Never write the bullet into `CHANGELOG.md` itself — its `## [Unreleased]` is assembled by the daily cut, which folds every fragment in, renames the section and deletes the files; every PR used to edit that one section and it conflicted on every `main` sync (#2295). `tests/test_changelog_integrity.py` validates each fragment and rejects an inline bullet with the fix. **Released `## [X.Y.Z]` blocks are frozen** — they are the record of what shipped, and a merge that lands your `[Unreleased]` bullet inside one (the common failure mode when a long-lived branch merges `main`) is a defect, not a formatting nit. `pyproject.toml`'s `[tool.agnes] released_changelog_sha256` pins a SHA-256 of the whole released region and `tests/test_changelog_integrity.py` fails on a mismatch. Fix `CHANGELOG.md`, not the checksum — `scripts/release_cut.py --rebaseline` re-stamps it and is for a deliberate, reviewed edit to shipped notes, never for clearing a red guard.
- **Release-cut is a dedicated cut PR, never a feature PR.** A feature/fix PR only ever adds a `changelog.d/` fragment — it never edits `CHANGELOG.md`, bumps `pyproject.toml`/`server.json`, or renames `[Unreleased]`. `.github/workflows/daily-cut.yml` cuts once a day (minor bump; `patch`/`major` on manual dispatch for a hotfix/milestone) into a PR labeled `release-cut` that a human reviews and merges — this is what killed the old CHANGELOG-rename race between competing feature PRs. After merge: `gh workflow run tag-release.yml -f tag=vX.Y.Z` (the cut PR's body carries the exact command) tags the merge commit and creates the GitHub Release.
...
- **What lands a ready PR is the merge queue, not a review badge and not a human train.** Since 2026-09-07 `main` has a GitHub merge queue (ruleset *Merge queue on main*: merge-commit method, every entry in a group must be green) and the "require branch up to date" rule is off. To land a ready PR: `gh pr merge <N> --merge --auto --delete-branch` (or "Merge when ready" in the UI). The queue builds `main` + your PR (+ whatever else is queued), runs the required checks — `test` and `docker-build`, which `ci.yml` runs on the `merge_group` event — on that merged result once, and merges. **Never merge `main` into a PR because `main` moved**; the queue does that, and a PR sitting BEHIND is normal. The review ruleset still decides who may queue (one approval plus an extra one for unattributed changes; the core team bypasses it), so what an agent waits on is a named person with bypass, never the badge ([`docs/RELEASING.md`](docs/RELEASING.md) → *Landing PRs through the merge queue*). The hand-driven `Train N: #…` merges are retired: CHANGELOG fragments removed the collisions that made the train's resolution role necessary.
- The daily release-cut PR goes through the same queue. Queue it after the feature PRs you want in that version have landed; a fragment merged after the cut PR opened simply rides the next day's cut, and a cut PR that conflicts with `main` is closed and re-dispatched, never hand-resolved (same section).
