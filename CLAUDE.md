...
- **What picks a ready PR up is the merge train, not GitHub.** Most of `main` arrives as `Train N: #…` — a driver sweeps the non-draft queue, reviews each PR, merges them onto a `train/<n>-queue` branch, resolves the `[Unreleased]` collisions the batch creates, verifies the whole tree, and lands it as one PR ([`docs/RELEASING.md`](docs/RELEASING.md) → *The train-driver role*). No workflow or ruleset merges anything on its own: `allow_auto_merge` only means the button exists, so a ready PR waits for the next train rather than for its required checks. Merging your own ready PR directly is allowed and happens for standalone work — just don't leave a half-finished branch non-draft assuming you still hold the decision.
- The daily release-cut PR is non-draft too but never rides a train: the driver flushes the queue first, then merges the cut PR (same section).
