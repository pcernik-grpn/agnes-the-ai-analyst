...
| New REST `/api/*` endpoint | smoke coverage on both app-state backends — add the `"METHOD /path"` to a `Test*Smoke.COVERED_ROUTES` set in `tests/db_pg/test_endpoints_smoke.py` with a test against `seeded_app_both` (a PG-only route asserts the typed `501 requires_postgres_backend` on DuckDB), or a justified `KNOWN_UNTESTED` entry there | BLOCKING | `tests/db_pg/test_endpoints_smoke.py::test_every_route_is_covered_or_excluded` — runs only in the `test-pg` CI shard, so no root-level guard or `verify_syncmap.py` warning catches a miss locally |
...
| Change to WHICH parquet layout wins for a table — the flat `data/<table>.parquet` or the sibling `data/<table>/` partition directory | change the ONE shared comparator, `src/parquet_publish.py`'s `partition_dir_supersedes_flat` (the fresher layout wins: any part with mtime `>=` the flat file's, so a tie goes to the directory), never a rule restated at a call site. Every precedence site must keep routing through it: `src/orchestrator.py`'s `_update_sync_state` (what the manifest advertises, i.e. what `agnes pull` ships) and `app/utils.py`'s `resolve_local_parquet_glob` + `local_parquet_size_bytes` + `resolve_local_layout_target` (what `/api/v2/schema`, `/api/v2/scan`, `/api/v2/sample`, the catalog and the profiler read). A second copy of the verdict makes the manifest and the read surfaces serve DIFFERENT data with nothing to say so — the failure mode #1339 was filed for, in a new direction. Only the LOSING flat sibling is ever reclaimed (`retire_superseded_parquet`), and only when the directory won; in the mirror direction nothing is deleted | BLOCKING | partial (`tests/test_parquet_retire_superseded.py` pins the comparator; `tests/test_orchestrator_sync_state_hash.py` + `tests/test_utils_parquet_layout_collision.py` pin each site in both directions; nothing statically proves a new site calls it) |
...
- **MCP-exposed:** a foundation tool in `app/api/mcp/foundation_tools.py` (the
  shared registry both remote transports serve), or a `tool_registry` passthrough
  row registered by `app/api/mcp/tools_generator.py`. The stdio server
  (`cli/mcp/server.py`) is a deliberately closed set pinned by
  `tests/test_mcp_tool_parity.py` — adding a tool there is a conscious test
  update, not the default route to MCP exposure.
